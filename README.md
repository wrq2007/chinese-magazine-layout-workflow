...
视觉复读、字体形态判断、图像理解、文案生成走 DeepSeek 的视觉与语言模型；
量尺寸、查重叠、验出血、跑版式引擎走脚本。

```
Codex（编排体）──┐                        ┌── dsh（视觉/文案体）
 版式引擎 / 验收 / 交付                   品牌文案 / 逐字复读 / 图像理解
                 └────► 同一台 InDesign ◄────┘
```

为什么这样分：**模型擅长"读内容"，不擅长"量几何"**。
实测视觉模型会把版面比例 16% 说成 48%，但它能准确读出屏幕上的字、判断标题是宋体还是黑体、
发现"右页下半幅被压成平黑"——两类任务交给两类工具，速度和准确率同时提升。

...
| **layout-qc** | 成品验收 | **十项量化检查**（含墨色色版、输出意图、图注专项）+ **错误档案**（真实踩过的坑）+ 工具脚本 |
...
这套工作流由**一个编排体（Codex）+ 两支辅助力量**组成：

| 角色 | 干什么 | 为什么是它 |
| --- | --- | --- |
| **Codex（编排 + 量测 + 验收）** | 跑版式引擎、写量测脚本、读图看观感、决定要不要问用户 | 它同时握着脚本、文件和判断权 |
| **本地多模态模型** | 审美第二意见、看图描述、文字审计初筛 | 不同模型家族、本地零费用 |
| **冷启动复核子代理** | 零上下文独立复核定稿 | 一个不知道我怎么想的审查者，用另一条路再量一遍 |

> **2026-09-20 改版**：此前这里写的是"双运行体"——Codex 编排，DeepSeek Harness（dsh）负责看图与写文案。
> 改版原因是 Codex 侧已经具备图像输入能力，"找人替我看"不再需要单独一个运行体；
> 而且两者走的是**同一个 API key**，换掉它并不省额度，只是少一个活动部件。
> 同一份成品、同一份盲审任务书的实测对照见
> [skills/layout-qc/references/review-loop.md](skills/layout-qc/references/review-loop.md)。

**模型擅长"看"，不擅长"量"，也不适合"认字"。** 实测视觉模型会把版面比例 16% 说成 48%；
它也不是可靠的认字工具——图片进模型前会被降采样，小字随之失真
（把署名「翟翊翔」读成「曹玥玥」、把图注「nomadict」读成「nomadicst」）。
...
```
Codex 排版 → 本地多模态模型审 → Codex 改 → 冷启动子代理复核（零上下文，不回调前一个）
```
...
**但比"用谁复核"更重要的是：复核者抓到的独家发现要立刻固化成 `qc-all` 的检查项。**
复核者每被吸收一次发现，边际价值就掉一截；只有把发现变成脚本，下一轮才不必重复同样的劳动。
2026-09-20 那次对照之后新增了三项检查（文字墨色色版构成、输出意图、图注对比度/残字/间距），
并给 TAC 加了"贴线"提醒。

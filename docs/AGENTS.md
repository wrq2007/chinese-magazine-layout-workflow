...
| 篇目 | 成品目录（.indd + 印刷 PDF） | 状态 |
| --- | --- | --- |
| 序言（2 页跨页） | `E:\杂志\序言\` | 已交付 |
| 发刊词（2 页跨页） | `E:\杂志\发刊词\` | 已交付 |
| 通透（4 页） | `E:\杂志\通透\` | 已交付；图注有 3 条待定（见印刷问题记录 §3） |

### 接手一个新对话，只需要知道三件事

1. 项目根目录是 `E:\杂志`（同目录的 `AGENTS.md` 会被自动读到，就是本文件）。
2. **待办与全部实测数据在 `E:\杂志\印刷问题记录.md`** —— 开工先读它，
   不要问用户"上次做到哪了"。
3. 印刷/工艺口径**还没定**（用户在跟印厂沟通中）：这类问题**只记录、不擅自改稿**。

用户说"继续"或直接给新需求即可，**不需要重新交代规格、规矩和历史**。

> ⚠️ 本文件每轮对话都会被重发一遍，**是每个请求的固定成本**。加内容请克制，
> 大段的经验与案例放到 `layout-qc/references/` 下面去。
